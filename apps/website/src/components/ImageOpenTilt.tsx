...
// transitions.dev Pro "image-open-tilt" — React variant, adapted to
// TypeScript. The scale spring and tilt live in src/styles/transitions.css;
// the organic bend is a software displacement warp drawn to a canvas layer
// (SVG displacement filters don't exist in Safari — feImage data-URI maps
// never load there).
...
function makeBendEase(raw: string): (x: number) => number {
...
function grid(g: Float32Array, x: number, y: number): number {
  ...
  fx = fx * fx * (3 - 2 * fx);
  fy = fy * fy * (3 - 2 * fy);
  const x0 = ((xi % NZ) + NZ) % NZ,
    x1 = (x0 + 1) % NZ;
  const y0 = ((yi % NZ) + NZ) % NZ,
    y1 = (y0 + 1) % NZ;
...
export function ImageOpenTilt({ src, alt = '', openLabel, closeLabel }: ImageOpenTiltProps) {
  ...
  const runBend = useCallback((sign: number, flightMs: number) => {
    ...
    // it, so the CSS box must be sized explicitly.
    ...
    try {
      snap = scx.getImageData(0, 0, sw, sh).data;
    } catch {
      // The photo tainted the canvas (no CORS) — skip the bend, the
      // scale/tilt flight still runs.
      return;
    }
    ...
    const render = (s: number, rCss: number) => {
      ...
      if (!st) return;
      // Radius passed in analytically — reading getComputedStyle here
      // every frame would force a Safari style recalc per frame.
      ...
      for (let y = 0; y < H; y++) {
        ...
        for (let x = 0; x < W; x++, di += 4) {
          ...
          const a = row0 + gx0,
            b = row1 + gx0;
          const Fx =
            (st.latFX[a] + (st.latFX[a + 1] - st.latFX[a]) * fxL) * (1 - fyL) +
            (st.latFX[b] + (st.latFX[b + 1] - st.latFX[b]) * fxL) * fyL;
          const Fy =
            (st.latFY[a] + (st.latFY[a + 1] - st.latFY[a]) * fxL) * (1 - fyL) +
          ...
          let sx0 = Math.floor(sxf),
            sy0 = Math.floor(syf);
          if (sx0 < 0) sx0 = 0;
          else if (sx0 > st.sw - 2) sx0 = st.sw - 2;
          if (sy0 < 0) sy0 = 0;
          else if (sy0 > st.sh - 2) sy0 = st.sh - 2;
          ...
          dst[di + 1] =
            srcD[i00 + 1] * w00 + srcD[i10 + 1] * w10 + srcD[i01 + 1] * w01 + srcD[i11 + 1] * w11;
          dst[di + 2] =
            srcD[i00 + 2] * w00 + srcD[i10 + 2] * w10 + srcD[i01 + 2] * w01 + srcD[i11 + 2] * w11;
    ...
    // duration + curve as the CSS border-radius transition. 32px rest ↔
    // 16px open — must match the .t-opentilt-card / .is-open CSS.
  ...
  return (
    <div className="t-opentilt-stage">
      <button
        ref={cardRef}
        type="button"
        className={'t-opentilt-card' + (open ? ' is-open' : '') + (closing ? ' is-closing' : '')}
        aria-expanded={open}
        aria-label={open ? closeLabel : openLabel}
        onClick={toggle}
      >
        {/* No crossOrigin: the placeholder CDN sends no CORS headers, and a
            tainted canvas only skips the optional bend warp (guarded). */}
        <img ref={imgRef} className="t-opentilt-img" src={src} alt={alt} loading="lazy" />
