...
describe('enforceDecryptedCacheLimit', () => {
  test('deletes oldest files first until the directory fits the cap', async () => {
    seedFile(`${DECRYPTED_CACHE_DIRECTORY}old-original.jpg`, new Uint8Array(40), 1);
    seedFile(`${DECRYPTED_CACHE_DIRECTORY}middle-original.jpg`, new Uint8Array(40), 2);
    seedFile(`${DECRYPTED_CACHE_DIRECTORY}new-original.jpg`, new Uint8Array(40), 3);

    await enforceDecryptedCacheLimit(80);

    expect(readFile(`${DECRYPTED_CACHE_DIRECTORY}old-original.jpg`)).toBeUndefined();
    expect(readFile(`${DECRYPTED_CACHE_DIRECTORY}middle-original.jpg`)).toBeDefined();
    expect(readFile(`${DECRYPTED_CACHE_DIRECTORY}new-original.jpg`)).toBeDefined();
  });

  test('leaves the directory alone while it is under the cap', async () => {
    seedFile(`${DECRYPTED_CACHE_DIRECTORY}a-original.jpg`, new Uint8Array(10), 1);
    seedFile(`${DECRYPTED_CACHE_DIRECTORY}b-original.jpg`, new Uint8Array(10), 2);

    await enforceDecryptedCacheLimit(80);

    expect(readFile(`${DECRYPTED_CACHE_DIRECTORY}a-original.jpg`)).toBeDefined();
    expect(readFile(`${DECRYPTED_CACHE_DIRECTORY}b-original.jpg`)).toBeDefined();
  });
});
