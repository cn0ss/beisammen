...
const ActivityInboxRow = memo(function ActivityInboxRow({
  ...
  return (
    <Pressable
      ...
      accessibilityLabel={`${item.displayText} Beitrag öffnen`}
      onPress={() => onOpen(item)}
      style={({ pressed }) => [
        styles.row,
        {
    ...
    >
      <Avatar name={item.actorName} imageUrl={item.actorAvatarUrl ?? null} size="sm" />
...
export default function ActivityScreen() {
  ...
  useEffect(() => {
    ...
    void markManyRead({ inboxItemIds: unreadVisibleIds }).catch((error) => {
      setFeedback(error instanceof Error ? error.message : 'Aktivität konnte nicht gelesen markiert werden.');
    });
  }, [markManyRead, unreadVisibleIds, unreadVisibleKey]);
  ...
  const handleOpenActivity = useCallback(
    async (item: ActivityInboxItemRecord) => {
      ...
      try {
        ...
        setFeedback(error instanceof Error ? error.message : 'Aktivität konnte nicht geöffnet werden.');
        return;
      }
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        {!hasViewer || isLoadingFirstPage ? (
          ...
          <EmptyState
            ...
            message="Sobald andere in deinen Circles teilen, kommentieren oder reagieren, erscheint es hier."
          />
        ) : (
          <View style={styles.list}>
            {inboxItems.map((item) => (
              <ActivityInboxRow key={item._id} item={item} onOpen={handleOpenActivity} />
            ))}
          </View>
        )}
